 ...
 * recorded on window.__pcs, so the test can read genuine WebRTC stats. The
...
function inboundStats(page: Page): Promise<PeerStats[]> {
  return page.evaluate(async () => {
    const all = (window as unknown as { __pcs: RTCPeerConnection[] }).__pcs;
    const results: { audioBytes: number; videoFrames: number }[] = [];
    for (const pc of all.filter((p) => p.connectionState === 'connected')) {
      const report = await pc.getStats();
      let audioBytes = 0;
      let videoFrames = 0;
      report.forEach(
        (r: { type: string; kind?: string; bytesReceived?: number; framesDecoded?: number }) => {
          if (r.type !== 'inbound-rtp') return;
          if (r.kind === 'audio') audioBytes += r.bytesReceived ?? 0;
          if (r.kind === 'video') videoFrames += r.framesDecoded ?? 0;
        },
      );
      results.push({ audioBytes, videoFrames });
    }
    return results;
  });
}
...
async function person(
  browser: Browser,
  request: Parameters<typeof createVerifiedAccount>[1],
  name: string,
) {
